...
tempDf <- merge(maize_bio_work, maize_N_work, by=c("HarvestDate","Plot","Organ"))
...
maize_bio_N_work %>%
  ...
  filter (VarName == "BiomassWeight") %>%
   ...
   geom_point() +
   geom_line() +
   facet_wrap(Organ~Block, scales="free")
...
maize_wheat_bio_N_work %>%
  dplyr::select(-Nitrogen, -Plot, -Nitrogen, -Irrigation, -Block) %>%
  ggplot(aes(x=Organ,y=BiomassWeight, colour=WaterTreat)) +
...
```{r}
write.csv(maize_wheat_bio_N_work,"C:\\GitHubRepos\\Arbitrator\\Analysis\\Biomass_N_merged_WheatMaize.csv")
```
